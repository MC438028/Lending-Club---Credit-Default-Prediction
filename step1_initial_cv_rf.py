...
class Step1CrossValidator:
    ...
    def _save_results(self):
        ...
        self.cv_summary_df.to_csv(self.output_dir / 'rq1_cv_metrics_rf.csv', index=False)
        print(f"交叉验证结果已保存: {self.output_dir / 'rq1_cv_metrics_rf.csv'}")

        # === 2. 测试集结果 ===
        test_df = pd.DataFrame(self.test_results_all.values())
        test_df.to_csv(self.output_dir / 'rq1_test_metrics_rf.csv', index=False)
        print(f"✓ 测试集结果已保存: {self.output_dir / 'rq1_test_metrics_rf.csv'}")


        # === 3. 保存完整结果 (PKL) - 包含预测值用于可视化 ===
        with open(self.output_dir / 'cv_results_detailed_rf.pkl', 'wb') as f:
            pickle.dump({
                'cv_results': self.cv_results_all,
                'test_results': self.test_results_all,
            }, f)
        print(f"✓ 详细结果已保存: {self.output_dir / 'cv_results_detailed_rf.pkl'}")
    ...
    def _calculate_rankings(self):
        ...
        # 保存 Top 4
        with open(self.output_dir / 'top4_subsets_rf.pkl', 'wb') as f:
            pickle.dump(self.top4_subsets, f)
        print(f"\n✓ Top 4 方法已保存: {self.output_dir / 'top4_subsets_rf.pkl'}")

        # 保存排序后的完整排名
        self.cv_summary_df.to_csv(
            self.output_dir / 'rq1_cv_metrics_ranked_rf.csv', index=False
        )
