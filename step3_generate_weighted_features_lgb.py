...
class FinalFeatureGenerator:
    def __init__(self, 
                 ...
                 weights_file="results/rq2_optimized_weights_modified.pkl",
                 output_dir="data"):  
    ...
    def _load_data(self):
        ...
        # 验证结果格式（对应step2中的after_removal(方法二)和after_replacement(方法三)）
        required_keys = ['after_removal', 'after_replacement']
        for key in required_keys:
            if key not in self.optimization_results:
                raise ValueError(f"权重文件缺少必要的 {key} 数据，请检查step2输出")
        
        # 提取方法二（删除全零子集）和方法三（替换全零子集）的关键信息
        self.results = {
            'after_removal': {  # 方法二：删除全零子集后重新计算
                'subset_weights': self.optimization_results['after_removal']['subset_weights'],
                'best_n_features': self.optimization_results['after_removal']['weighted_feature_count']
            },
            'after_replacement': {  # 方法三：替换全零子集为下一名后计算
                'subset_weights': self.optimization_results['after_replacement']['subset_weights'],
                'best_n_features': self.optimization_results['after_replacement']['weighted_feature_count']
            }
        }
        
        # 打印每个方法的单个子集权重（带占比）
        for method in ['after_removal', 'after_replacement']:
            print(f"\n{method}（方法二/三）运用到的单个子集权重:")
            print("-"*40)
...
if __name__ == "__main__":
    ...
    generator = FinalFeatureGenerator(
        ...
        weights_file="/Users/xumoyan/Program/anaconda3/envs/cisc7201/final report/ML/Project/results/rq2_optimized_weights_modified.pkl",  # 对应step2的输出文件
        output_dir="/Users/xumoyan/Program/anaconda3/envs/cisc7201/final report/ML/Project/data"
    )
    
    # 为方法二（after_removal：删除全零子集）生成特征集
    print("\n" + "="*80)
    print("===== 开始生成 方法二（after_removal）特征集 =====")
    print("="*80)
    # 方法二最佳数量特征集
    method2_best_count = generator.results['after_removal']['best_n_features']
    method2_best_features, method2_best_name = generator.generate_features(
        n_features=method2_best_count,
        method='after_removal'
    )
    
    # 方法二固定42个特征集（用于对比）
    method2_42_features, method2_42_name = generator.generate_features(
        n_features=42,
        method='after_removal'
    )
    
    # 为方法三（after_replacement：替换全零子集）生成特征集
    print("\n" + "="*80)
    print("===== 开始生成 方法三（after_replacement）特征集 =====")
    print("="*80)
    # 方法三最佳数量特征集
    method3_best_count = generator.results['after_replacement']['best_n_features']
    method3_best_features, method3_best_name = generator.generate_features(
        n_features=method3_best_count,
        method='after_replacement'
    )
    
    # 方法三固定42个特征集（用于对比）
    method3_42_features, method3_42_name = generator.generate_features(
        n_features=42,
        method='after_replacement'
    )
    ...
    print("特征集生成完成，文件清单：")
    print("="*80)
    print(f"1. {method2_best_name}: after_removal_features_{method2_best_count}.pkl ({len(method2_best_features)}个特征)")
    print(f"2. {method2_42_name}: after_removal_features_42.pkl ({len(method2_42_features)}个特征)")
    print(f"3. {method3_best_name}: after_replacement_features_{method3_best_count}.pkl ({len(method3_best_features)}个特征)")
    print(f"4. {method3_42_name}: after_replacement_features_42.pkl ({len(method3_42_features)}个特征)")
