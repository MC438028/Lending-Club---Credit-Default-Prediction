...
def process_categorical_columns(cleaned_data):
    ...
    encoded_array = ohe.fit_transform(cleaned_data[existing_cols])  # 编码结果为二维数组（每行是一个向量）
    ...
    for col_idx, col in enumerate(existing_cols):
        ...
        cleaned_data[f"{col}_onehot_vector"] = [str(encoded_array[row, start:end].tolist()) for row in range(len(cleaned_data))]
        # 删除原始分类列
    ...
    print(cleaned_data[vector_cols].head())
