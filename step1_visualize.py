...
class RQ1Visualizer:
    ...
    def _load_results(self):
        ...
        # 加载交叉验证汇总
        self.cv_summary = pd.read_csv(self.results_dir / 'rq1_cv_metrics.csv')
        print(f"加载交叉验证结果: {len(self.cv_summary)} 个方法")
        
        # 加载测试集结果
        self.test_results = pd.read_csv(self.results_dir / 'rq1_test_metrics.csv')
        print(f"加载测试集结果: {len(self.test_results)} 个方法")
        
        # 加载详细结果
        with open(self.results_dir / 'cv_results_detailed.pkl', 'rb') as f:
    ...
    def plot_cv_roc(self, save=True):
        ...
        # 标题
        plt.title("Cross-Validation Mean ROC Curves for Different Feature Subsets(LightGBM)")
        plt.legend(loc="lower right")
        plt.grid(False)
        
        if save:
            save_path = self.fig_dir / 'rq1_cv_mean_roc.png'
    ...
    def plot_cv_combined(self, save=True):
            ...
            if save:
                save_path = self.fig_dir / 'rq1_cv_combined_visualization.png'
                plt.savefig(save_path, dpi=300, bbox_inches="tight")
                print(f"✓ 已保存: {save_path}")
    ...
    def plot_test_comparison(self, save=True):
        ...
        roc_data = pd.DataFrame({
            ...
            'ROC-AUC': [round(self.test_results_detailed[k]['Test_ROC_AUC'], 3) for k in self.test_results_detailed],
        ...
        # 添加数值标签在柱状图上
        plt.subplot(1, 2, 2)
        barplot = sns.barplot(x='Score', y='Subset', hue='Metric', data=comparison_data, palette='Set2')
        for container in barplot.containers:
            barplot.bar_label(container, fmt="%.3f", fontsize=7, padding=3)

            # 【修改2】扩展右侧x轴范围，视觉上让x轴更长，给标签留足空间
        max_score = comparison_data['Score'].max()
        plt.xlim(0, max_score + 0.15)  
        ...
        if save:
            save_path = self.fig_dir / 'rq1_test_performance_comparison.png'
            plt.savefig(save_path, dpi=300, bbox_inches="tight")
            print(f"✓ 已保存: {save_path}")
    ...
    def plot_test_roc(self, save=True):
        ...
        plt.ylabel('True Positive Rate')
        # 标题
        plt.title('ROC Curve Comparison for Different Feature Subsets(LightGBM)')
        plt.legend(loc='lower right')
        plt.tight_layout()
        
        if save:
            save_path = self.fig_dir / 'rq1_test_roc.png'
            plt.savefig(save_path, dpi=300, bbox_inches="tight")
            print(f"✓ 已保存: {save_path}")
