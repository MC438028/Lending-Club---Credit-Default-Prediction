...
train_raw_path = os.path.join(dataset_dir, 'train_data.csv')

if not os.path.exists(train_raw_path):
    raise FileNotFoundError(f"请先运行数据管道生成 {train_raw_path}")

# 读取原始训练数据（包含特征和目标列）
train_data = pd.read_csv(train_raw_path, low_memory=False)
print(f"读取原始训练集成功，形状: {train_data.shape}")
...
def process_binary_columns(train_data):
    ...
    display_cols = list(manual_mappings.keys()) + existing_remaining 
